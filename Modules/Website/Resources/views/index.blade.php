...
{{--                </div>--}}
                @foreach($sliders as $key => $slider)
                    ...
                    @else
                        <div class="carousel-item {{ $key == 0 ? 'active' : '' }}">
                            <video class="carousel-video" controls autoplay allowfullscreen muted>
                                <source src="{{ $slider->picture->url }}" type="video/mp4">
                                Your browser does not support the video tag.
                            </video>
{{--                            <iframe class="carousel-video" src="{{ $slider->picture->url }}?autoplay=1" frameborder="0" allowfullscreen></iframe>--}}
                        </div>
                    @endif
                @endforeach
            </div>
                ...
                @foreach($processes as $process)
                    <div class="col-md-3 col-sm-6 my-2 p_column text-center mt-3">
                        <a class="text-decoration-none text-white" id="btnModalProduct" data-image="{{ $process->picture->url }}">
                            <h3 class="fw-bold">STEP {{ $process->step }} - {{ $process->title }}</h3>
